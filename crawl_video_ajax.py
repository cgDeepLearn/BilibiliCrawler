...
def crawl2csv(filename, start, end):
    ...
    with open(filename, 'w', encoding='utf8', newline='') as fwriter:
        ...
        for _ in range(consumer_num):
            cthread = Consumer(Q, csvwriter=mycsvwriter, func=BiliVideo.store_video_simpleajax, sleepsec=0.01)
            mythreads.append(cthread)
        with Timer() as t:
            for thread in mythreads:
                thread.start()
...
if __name__ == '__main__':
    ...
    if mode == 'file':
        filepath = os.path.join(BASE_DIR, 'data/videoinfo_ajax_%s_%s.csv' % (start, end))
